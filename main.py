...
class Company:
	def __init__(self, instrument_name):
		...
		self.industryMean = self.industryMean.sort_values(by=['ESG Combined Score'], kind='quicksort')  # second step
		...
		self.industryMeanR = self.industryMeanR.sort_values(by=['ESG Score'], kind='quicksort')  # second step
		...
		self.industryMeanEnvironmental = self.industryMeanEnvironmental.sort_values(by=['Environmental Pillar Score'], kind='quicksort')  # second step
		...
		self.industryMeanSocial = self.industryMeanSocial.sort_values(by=['Social Pillar Score'], kind='quicksort')  # second step
		...
		self.industryMeanGovernance = self.industryMeanGovernance.sort_values(by=['Governance Pillar Score'], kind='quicksort')  # second step
		...
		self.countryMean = self.countryMean.sort_values(by=['ESG Combined Score'], kind='quicksort')  # second step
		...
		self.countryMeanR = self.countryMeanR.sort_values(by=['ESG Score'], kind='quicksort')  # second step
		...
		self.countryMeanEnvironmental = self.countryMeanEnvironmental.sort_values(by=['Environmental Pillar Score'], kind='quicksort')  # second step
		...
		self.countryMeanSocial = self.countryMeanSocial.sort_values(by=['Social Pillar Score'], kind='quicksort')  # second step
		...
		self.countryMeanGovernance = self.countryMeanGovernance.sort_values(by=['Governance Pillar Score'], kind='quicksort')  # second step
	...
	@staticmethod
	def create_list(number_of_elements, retrieved_list):
		array_of_elements = []
	...
	@staticmethod
	def create_list_remove_nas(number_of_elements, retrieved_list):
		array_of_elements = []
		for i in range(number_of_elements):
	...
	@staticmethod
	def get_pos(retrieved_list, value_of_pos):
		pos = 0
			...
			html.Div([
				dcc.Graph(
					...
					figure={
						'data': [
							go.Bar(
								x=compV.industry_list,
								y=compV.industry_esg_list,
								marker_color='#0cedb9'
							)
						],
						'layout' :
							go.Layout(
								title="Performance within " + compV.industry,
...
@app.callback(
	...
	Output(component_id='custom_score_text', component_property='value'),
...
def update(option_slcted, esg_slected, comp_input, e, s, g):
	...
	if e > 0 and s > 0 and g > 0:
		custom_esg = "Custom Score: \n" + str(round(compZ.own_score(e, s, g), 2))
	else:
		custom_esg = "No valid entry"
	...
	if option_slcted == "Industry":
		if esg_slected == "ESG Combined":
			...
			esg_score_txt = 'Score: ' + str(round(compZ.esg_value, 2)) + '\n' + 'Rel: ' + str(round(comp_relative, 2))
			...
			industry_or_company = compZ.industry
			esg_score_txt = 'Score: ' + str(round(compZ.esg_value_r, 2)) + '\n' + 'Rel: ' + str(round(comp_relative, 2))
			...
			industry_or_company = compZ.industry
			esg_score_txt = 'Score: ' + str(round(compZ.environmental, 2)) + '\n' + 'Rel: ' + str(round(comp_relative, 2))
			...
			industry_or_company = compZ.industry
			esg_score_txt = 'Score: ' + str(round(compZ.social, 2)) + '\n' + 'Rel: ' + str(round(comp_relative, 2))
		else:
			indus_or_country_list_len = compZ.industry_esg_list_len
			...
			industry_or_company = compZ.industry
			esg_score_txt = 'Score: ' + str(round(compZ.governance, 2)) + '\n' + 'Rel: ' + str(round(comp_relative, 2))
	else:
		if esg_slected == "ESG Combined":
	...
	return fig, esg_score_txt, custom_esg


if __name__ == '__main__':
	app.run_server(port='1002')
